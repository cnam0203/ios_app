...
class DashboardGSNN1 extends Dashboard2018 {
	...
	public function index ($type='') {
		...
		// 			'ajaxURL' => '/gsnreport/dashboardgsnn1?sync=1',
	...
	function _prepairData () {
		if ($this->IsAdmin == 1)
			$sql = $this->sqlListDashboardGSN('N1Total');
		else
			$sql = $this->sqlListDashboardGSN_notfull('N1Total');
	...
	function _prepairDataNotList () {
		$sql = $this->sqlNotListDashboardGSN('N1Total');
		$data = $this->getDataPDOSQL($this->pdoReportTool,$sql);
	...
	function jsonWidgets () {
		...
		// // configured list
		// $data = $this->_prepairData();
		// $html .= $this->showWidgets($data, 'N1Total', $maxWidgetPerRow, $currentRow, $currentCol);
