...
class DashboardGSNA1 extends Dashboard2018 {
	...
	public function index ($type='') {
		...
		// 			'ajaxURL' => '/gsnreport/dashboardgsna1?sync=1',
	...
	function _prepairData () {
		if ($this->IsAdmin == 1)
			$sql = $this->sqlListDashboardGSN('A1Total');
		else
			$sql = $this->sqlListDashboardGSN_notfull('A1Total');
	...
	function _prepairDataNotList () {
		$sql = $this->sqlNotListDashboardGSN('A1Total');
		$data = $this->getDataPDOSQL($this->pdoReportTool,$sql);
	...
	function jsonWidgets () {
		...
		// // configured list
		// $data = $this->_prepairData();
		// $html .= $this->showWidgets($data, 'A1Total', $maxWidgetPerRow, $currentRow, $currentCol);
	...
	function jsonWidgets_notfull () {
		...
		// configured list
		$data = $this->_prepairData();
		$this->showWidgets($data, 'A1Total', $maxWidgetPerRow, $currentRow, $currentCol);
